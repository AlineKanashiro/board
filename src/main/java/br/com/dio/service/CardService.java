...
public class CardService {
    ...
    public void moveToNextColumn(final Long cardId, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException {
        try {
            ...
            var dto = optional.orElseThrow(() -> new EntityNotFoundException("O card de ID %s nao foi encontrado".formatted(cardId)));
            if (dto.blocked()) {
                throw new CardBlockedException("O card %s esta bloqueado, e necessario desbloquea-lo para mover".formatted(cardId));
            }
    ...
    public void cancel(final Long cardId, final Long cancelColumnId, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException {
        try {
            var dao = new CardDAO(connection);
            var optional = dao.findById(cardId);
            var dto = optional.orElseThrow(() -> new EntityNotFoundException("O card de ID %s não foi encontrado"));
            if (dto.blocked()) {
                var message = "O card %s esta bloqueado, e necessario desbloquea-lo para mover".formatted(cardId);
    ...
    public void block(final Long id, final String reason, final List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException {
        try {
            ...
            if (dto.blocked()) {
                throw new CardBlockedException("O card %s ja esta bloqueado".formatted(id));
            }
